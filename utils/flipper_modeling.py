...
__all__ = ["get_flipper_pointwise_vels", "FlipperWheels"]


class FlipperWheels(TypedDict):
    """
    Represents the wheels of the flipper.
...
def get_flipper_pointwise_vels(
    flipper_points: torch.Tensor,
    track_wheels: FlipperWheels,
    ...
    for i in range(0, N, 2):
        ...
        if i + 1 < N:  # If there is a next wheel
            ...
            vels[assigned] = torch.cross(
                wheel_rot_vec.unsqueeze(0), (R.T @ diff_vecs[assigned].T).T
            )
            used[assigned] = True
            ...
            inbetween_mask = (
                torch.sum(remaining_pts_diffs_i.view(-1, 3) * wheel_center_diff, dim=1) > 0
            ...
            center_diff_normal = normalized(
                torch.tensor([wheel_center_diff[1], -wheel_center_diff[0], 0])
            )
            # Todo fix by subtracting the center between wheels from the rotated points
            left_mask = (
                torch.sum(rot_flipper_pts * center_diff_normal.view(-1, 3), dim=1) > flat_min_dist
            )
            remaining_mask = ~used
            vels[remaining_mask & inbetween_mask & left_mask] = R.T @ v_u
            vels[remaining_mask & inbetween_mask & ~left_mask] = R.T @ v_v
...
def get_wheel_point_diff_vecs(
    ...
    within_wheel = points_within_circle(
        points[:, :2], wheel_position[:2], wheel_radius, wheel_assignment_margin
    )
