...
from torchrl.modules import NormalParamExtractor, ProbabilisticActor, TanhNormal, ValueOperator, ActorCriticWrapper, ActorValueOperator, GRUModule
...
@dataclass
class GRUPolicyConfig(PolicyConfig):
    share_encoder: bool
    actor_mlp_opts: dict
    value_mlp_opts: dict
    ...
    def create(self, env: Env, **kwargs):
        ...
        GRUPolicyConfig._log_parameter_counts(actor_value_wrapper, self.share_encoder)
    ...
    def _create_separate(self, action_spec, combined_encoder: EncoderCombiner):
        ...
        actor_module = ProbabilisticActor(
            module=TensorDictSequential([actor_encoder_module, actor_gru, actor_module]),
            spec=action_spec,
            in_keys=["loc", "scale"],
        ...
        value_module = TensorDictModule(
            ...
            in_keys=["y_value_gru"],  # pass the observations as input
            out_keys=["state_value"],
        )
        value_operator = TensorDictSequential(
            [value_encoder_module, value_gru, value_module],
        )
        ...
        return wrapper, [actor_gru.make_tensordict_primer(), value_gru.make_tensordict_primer()]
    ...
    def _create_shared(self, action_spec, combined_encoder: EncoderCombiner):
        ...
        value_operator = ValueOperator(
            ...
            in_keys=["y_shared_gru"],  # pass the observations as input
        )
        ...
        gru_module = GRUModule(
            ...
            hidden_size=combined_encoder.output_dim,
            in_key="y_shared",
            out_key="y_shared_gru",
            default_recurrent_mode=True,
            **self.gru_opts,
        )
        common_module = TensorDictSequential([encoder_module, gru_module])
    ...
    def _log_parameter_counts(actor_value_wrapper, share_encoder: bool):
        ...
        if share_encoder:
            ...
            gru_params = count_parameters(actor_value_wrapper.module[0].module[1])
            ...
            table.add_row("Shared GRU Module", f"{gru_params:,}")
            ...
            actor_gru_params = count_parameters(policy_operator.module[0].module[1])
            ...
            value_gru_params = count_parameters(value_operator[1])
            value_head_params = count_parameters(value_operator[2])
            table.add_row("Actor Encoder", f"{actor_encoder_params:,}")
            table.add_row("Actor GRU Module", f"{actor_gru_params:,}")
            table.add_row("Actor Head", f"{actor_head_params:,}")
            table.add_row("Value Encoder", f"{value_encoder_params:,}")
            table.add_row("Value Head", f"{value_head_params:,}")
            table.add_row("Value GRU Module", f"{value_gru_params:,}")
