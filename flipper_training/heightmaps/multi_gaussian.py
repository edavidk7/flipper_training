...
@dataclass
class SingleMultiGaussianHeightmapGenerator(BaseHeightmapGenerator):
    """
    Generates a single heightmap using multiple gaussians.
    """
    ...
    def _generate_heightmap(self, x, y, max_coord, rng):
        ...
        z_i = torch.sum(
            ...
            * torch.exp(
                -(
                    (x[None, 0] - mus[0].view(-1, 1, 1)) ** 2 / (2 * sigmas[..., 0].view(-1, 1, 1) ** 2)
                    + (y[None, 0] - mus[1].view(-1, 1, 1)) ** 2 / (2 * sigmas[..., 1].view(-1, 1, 1) ** 2)
                )
            ),
            dim=0,
        )
