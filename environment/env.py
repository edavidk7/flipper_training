...
class Env(EnvBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.start = PhysicsState.dummy(
            batch_size=self.n_robots, robot_model=robot_model_config, device=self.device
        )
        self.goal = PhysicsState.dummy(
            batch_size=self.n_robots, robot_model=robot_model_config, device=self.device
        )
        # Specs
    ...
    def world_config(self, world_config: WorldConfig):
        assert world_config.suitable_mask is not None, (
            "Suitable mask is required for the world configuration"
        )
        ...
        match self.env_cfg.control_type:
            case "per-track":
                track_low = torch.full(
                    (self.n_robots, self.robot_cfg.num_joints),
                    -self.robot_cfg.vel_max,
                    device=self.device,
                )
                track_high = -track_low
            case "lon_ang":
                track_low = torch.tensor(
                    [-self.robot_cfg.vel_max, -self.robot_cfg.omega_max], device=self.device
                ).repeat(1, self.n_robots)
                track_high = -track_low
            case _:
    ...
    def _get_observations(
        ...
        return TensorDict(
            {
                k: obs(prev_state, action, state_der, curr_state, aux_info)
    ...
    def _compute_full_controls(self, controls: torch.Tensor) -> torch.Tensor:
        ...
        if controls.shape[1] == 2 * self.robot_cfg.num_joints:
            ...
            full_controls = torch.zeros(
                (self.phys_cfg.num_robots, 2 * self.robot_cfg.num_joints), device=self.device
            )
            full_controls[..., : self.robot_cfg.num_joints] = self.robot_cfg.get_controls(
                controls[:, :2]
            ).unsqueeze(0)
            full_controls[..., self.robot_cfg.num_joints :] = controls[:, 2:]
        else:
            raise ValueError(
                f"Invalid shape for controls: {controls.shape}. Expected {(self.phys_cfg.num_robots, 2 + self.robot_cfg.num_joints)} or {(self.phys_cfg.num_robots, 2 * self.robot_cfg.num_joints)}."
            )
        return full_controls

    def _step_engine(
        self, action: torch.Tensor
    ) -> tuple[PhysicsState, PhysicsStateDer, AuxEngineInfo, PhysicsState]:
        ...
        if self.env_cfg.differentiable:
            ...
            with torch.no_grad():
                next_state, state_der, aux_info = self.engine(
                    self.state, full_controls, self.world_cfg
                )
    ...
    def _reset(self, tensordict=None, **kwargs) -> TensorDict:
        ...
        new_start, new_goal = self.objective.generate_start_goal_states(
            self.world_cfg, self.robot_cfg, self.rng, skip_mask=skip_mask.unsqueeze(-1)
        )
        new_iteration_limits = self.objective.compute_iteration_limits(
            self.start, self.goal, self.robot_cfg, self.phys_cfg.dt
        )
    ...
    def _step(self, tensordict) -> TensorDict:
        ...
        obs_td["reward"] = self.reward(
            prev_state, action, state_der, next_state, aux_info, reached_goal, failed, self
        )
        self.step_count += 1
        return obs_td
